...
// dengan nama acak yang unik. Mengembalikan nama final + path relatif untuk URL.
async function finalize(stagedPath, originalName, uploadType) {
  const ext = path.extname(originalName).slice(1).toLowerCase();
  const dir = uploadType === 'temporary' ? TEMP_DIR : UPLOAD_DIR;
  const { name, target } = await uniqueTarget(dir, ext);
  await rename(stagedPath, target);
  const relative = uploadType === 'temporary' ? `uploads/temp/${name}` : `uploads/${name}`;
...
export async function handleUpload(parts) {
  ...
  try {
    ...
    let uploadType = fields.upload_type === 'permanent' ? 'permanent' : 'temporary';
    const result = await finalize(staged.path, staged.originalName, uploadType);
